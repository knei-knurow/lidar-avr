...
#define UART_Printf  // Just don't use any UART stuff here -- dsonyy

// Set initial input parameters
enum Ascale_values { AFS_2G = 0, AFS_4G, AFS_8G, AFS_16G };

enum Gscale_values { GFS_250DPS = 0, GFS_500DPS, GFS_1000DPS, GFS_2000DPS };
...
float zeta =
    0.8660254 *
    GyroMeasDrift;  // sqrt(3.0f / 4.0f) * GyroMeasDrift;   // compute zeta, the other free
...
void mpu9250_setup() {
  ...
  if (whoami == 0x71)  // WHO_AM_I should always be 0x68
  {
    ...
    UART_Printf("%d %d %d mg\n\r", (int)(1000 * accelBias[0]), (int)(1000 * accelBias[1]),
    ...
    while (BypassTrue == 0) {
      ...
      whoami = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
      UART_Printf(
          "PinCFG: %x = 0x22 MasterDisable %x = 0x00 Interrupts %x = 0x01 Whoami %x = 0x71\n\r",
          PinCFG, MasterDis, IntEna, whoami);
...
//===================================================================================================================
//====== Set of useful function to access acceleration. gyroscope, magnetometer, and temperature
// data
//===================================================================================================================
...
void initAK8963(float* destination) {
  ...
  destination[0] =
      (float)(rawData[0] - 128) / 256. + 1.;  // Return x-axis sensitivity adjustment values, etc.
  ...
  // and enable continuous mode data acquisition Mmode (bits [3:0]), 0010 for 8 Hz and 0110 for 100
  // Hz sample rates
...
void initMPU9250() {
  ...
  // minimum _delay_ms time for this setting is 5.9 ms, which means sensor fusion update rates
  // cannot be higher than 1 / 0.0059 = 170 Hz DLPF_CFG = bits 2:0 = 011; this limits the sample
  // rate to 1000 Hz for both With the MPU9250, it is possible to get gyro sample rates of 32 kHz
  // (!), 8 kHz, or 1 kHz
  ...
  // writeRegister(GYRO_CONFIG, c | 0x00); // Set Fchoice for the gyro to 11 by writing its inverse
  ...
  // Set interrupt pin active high, push-pull, hold interrupt pin level HIGH until interrupt
  // cleared, clear on read of INT_STATUS, and enable I2C_BYPASS_EN so additional chips can join the
  // I2C bus and all can be controlled by the Arduino as master
  writeByte(MPU9250_ADDRESS, INT_PIN_CFG, 0x22);
  writeByte(MPU9250_ADDRESS, INT_ENABLE, 0x01);  // Enable data ready (bit 0) interrupt
  _delay_ms(100);
}
...
// the average of the at-rest readings and then loads the resulting offsets into accelerometer and
// gyro bias registers.
void calibrateMPU9250(float* dest1, float* dest2) {
  ...
  // registers contain factory trim values which must be added to the calculated accelerometer
  // biases; on boot up these registers will hold non-zero values. In addition, bit 0 of the lower
  // byte must be preserved since it is used for temperature compensation calculations.
  // Accelerometer bias registers expect bias input as 2048 LSB per g, so that the accelerometer
                                          ...
                                          // values, +/- 14 or less deviation is a pass
{
  ...
  for (int ii = 0; ii < 3;
       ii++) {  // Get average of 200 values and store as average current readings
    aAvg[ii] /= 200;
    gAvg[ii] /= 200;
  }
  ...
  for (int ii = 0; ii < 3;
       ii++) {  // Get average of 200 values and store as average self-test readings
    aSTAvg[ii] /= 200;
    gSTAvg[ii] /= 200;
  }
  ...
  // Response To get percent, must multiply by 100
  for (int i = 0; i < 3; i++) {
    destination[i] =
        100.0 * ((float)(aSTAvg[i] - aAvg[i])) / factoryTrim[i];  // Report percent differences
    destination[i + 3] =
...
void MadgwickQuaternionUpdate(float ax,
  ...
  float q1 = quaternionBuffer[0], q2 = quaternionBuffer[1], q3 = quaternionBuffer[2],
        q4 = quaternionBuffer[3];  // short name local variable for readability
  float norm;
...
void writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
  ...
  ui8_return =
#endif
      twi_writeTo(address, pui8_sendbuffer, 2, 1,
                  (uint8_t) true);  // Send to address content of *subAddress
...
void readBytes(uint8_t address, uint8_t subAddress, uint8_t byteCount, uint8_t* destinationBuffer) {
  ...
  // connection alive ui8_return = i2c_rep_start(address + I2C_READ);
  ui8_return = twi_writeTo(address, &subAddress, 1, 1,
                           (uint8_t) false);  // Send to address content of *subAddress
                                              // with length 1 , wait for end of transmit and
  ...
  ui8_return = twi_readFrom(address, destinationBuffer, byteCount,
                            (uint8_t) true);  // Read byteCount-Bytes from
...
void readBytes_Debug(uint8_t address,
                     uint8_t subAddress,
                     uint8_t byteCount,
...
void magcalMPU9250(float* dest1, float* dest2) {
  ...
  dest1[0] =
      (float)mag_bias[0] * mRes * magCalibration[0];  // save mag biases in G for main program
