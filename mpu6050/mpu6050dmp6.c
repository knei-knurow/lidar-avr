...
/*
 * This file contains all the functions needed to process 6-axis orientation by the internal chip
 * processor
 */

// to enable get roll, pitch and yaw function we include the math function
// if some error appears we must add -lm and -lc to C linker, the linker line should become
// somethink like this
  ...
  if (mpu6050_writeMemoryBlock(mpu6050_dmpMemory, MPU6050_DMP_CODE_SIZE, 0, 0, 1, 1) == 1) {
    if (mpu6050_writeDMPConfigurationSet(mpu6050_dmpConfig, MPU6050_DMP_CONFIG_SIZE, 1)) {
      ...
      mpu6050_writeBits(MPU6050_RA_CONFIG, MPU6050_CFG_EXT_SYNC_SET_BIT,
                        MPU6050_CFG_EXT_SYNC_SET_LENGTH, MPU6050_EXT_SYNC_TEMP_OUT_L);
 ...
 */
void mpu6050_getRollPitchYaw(double qw,
                             double qx,
                             double qy,
                             double qz,
                             double* roll,
