  ...
  for (i = 0; i < dataSize;) {
    ...
    if (length > 0) {
      ...
      if (special == 0x01) {
        ...
        // mpu6050_writeBit(MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_ZMOT_BIT, 1);
        // //setIntZeroMotionEnabled mpu6050_writeBit(MPU6050_RA_INT_ENABLE,
        // MPU6050_INTERRUPT_FIFO_OFLOW_BIT, 1); //setIntFIFOBufferOverflowEnabled
        // mpu6050_writeBit(MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DMP_INT_BIT, 1);
        // //setIntDMPEnabled
...
void mpu6050_init() {
  ...
  // set clock source
  //  it is highly recommended that the device be configured to use one of the gyroscopes (or an
  //  external clock source) as the clock reference for improved stability
  mpu6050_writeBits(MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH,
                    MPU6050_CLOCK_PLL_XGYRO);
  // set DLPF bandwidth to 42Hz
  mpu6050_writeBits(MPU6050_RA_CONFIG, MPU6050_CFG_DLPF_CFG_BIT, MPU6050_CFG_DLPF_CFG_LENGTH,
                    MPU6050_DLPF_BW_42);
  // set sampe rate
 ...
 */
void mpu6050_getConvData(double* axg,
                         double* ayg,
                         double* azg,
                         double* gxds,
                         double* gyds,
...
void mpu6050_mahonyUpdate(float gx, float gy, float gz, float ax, float ay, float az) {
  ...
  if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
    ...
    if (mpu6050_mahonytwoKiDef > 0.0f) {
      integralFBx += mpu6050_mahonytwoKiDef * halfex *
                     (1.0f / mpu6050_mahonysampleFreq);  // integral error scaled by Ki
